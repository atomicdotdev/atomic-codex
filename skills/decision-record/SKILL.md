...
At the end of a turn, look back over what happened — the ledger (`atomic change`,
...
## 3. Record each — `new → attest → validate → verify`
...
**Attest first, then validate and verify.** `attest` runs the full gate itself before
...
- **Always `attest`, then `validate` and `verify`.** An unattested memory is
  unsigned and untrusted — finish all three memory steps. `attest` gates before it signs;
...
- **Do NOT run `atomic add` or `atomic record`.** `atomic memory new` writes into
  the vault and the Codex hooks record it automatically at turn end (same as
...
- **Always attest and verify.** Run `atomic memory attest`, `atomic memory
  validate`, then `atomic memory verify` after each `new`.
